...
A binary search tree (BST) implemented here has following methods:
  ...
  - **_depthFirstTraversal(order)_** - traverses through all nodes according to the ```order``` parameter and logs them to the console. Possible values of the ```order``` are:
    - _in-order_ - starts traversing the tree from the top and follows every branch down to its bottom before moves on to the next branch.
    - _pre-order_ - starts traversing the tree from the top, then goes down the left branch and then goes down the right branch.
